...
from preprocessing.derived_features import create_derived_features
...
def run_preprocessing(config: Dict[str, Any]) -> Path:
    ...
    df = create_derived_features(df, config)
    logger.info(f"Feature derivate completate -> cols={len(df.columns)}")
