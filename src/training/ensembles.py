...
def build_voting(models_best: List[Tuple[str, Dict[str, Any]]], tune_weights: bool = True) -> VotingRegressor:
...
def build_stacking(models_best: List[Tuple[str, Dict[str, Any]]], final_estimator_key: str, cv_folds: int = 5) -> StackingRegressor:
    estimators = [(f"{k}", build_estimator(k, p)) for k, p in models_best]
